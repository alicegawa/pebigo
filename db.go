...
func (db *DB) Delete(wo *WriteOptions, key []byte) error {
	...
	C.leveldb_delete(db.Pdb, wo.Opt, k, C.size_t(len(key)), &errStr)
...
func (db *DB) Write(wo *WriteOptions, w *WriteBatch) error {
	...
	var errStr *C.char
	C.leveldb_write(db.Pdb, wo.Opt, w.wbatch, &errStr)
	if errStr != nil {
		gs := C.GoString(errStr)
		C.leveldb_free(unsafe.Pointer(errStr))
		return DatabaseError(gs)
...
func (db *DB) NewIterator(ro *ReadOptions) *Iterator {
	...
	it := C.leveldb_create_iterator(db.Pdb, ro.Opt)
...
func (db *DB) GetApproximateSizes(ranges []Range) []uint64 {
	...
	C.pebigo_leveldb_approximate_sizes(db.Pdb, numranges, startsPtr, startLensPtr, limitsPtr, limitLensPtr, sizesPtr)
	for i := range ranges {
...
func (db *DB) PropertyValue(propName string) string {
	...
	value := C.GoString(C.leveldb_property_value(db.Pdb, cname))
...
func (db *DB) NewSnapshot() *Snapshot {
	...
	return &Snapshot{C.leveldb_create_snapshot(db.Pdb)}
}
...
func (db *DB) ReleaseSnapshot(snap *Snapshot) {
	...
	C.leveldb_release_snapshot(db.Pdb, snap.snap)
}
...
func (db *DB) CompactRange(r Range) {
	...
	C.leveldb_compact_range(db.Pdb, start, C.size_t(len(r.Start)), limit, C.size_t(len(r.Limit)))
}
